...
static inline void calc_tilt(wiimote_t *wiimote)
{
	...
	wiimote->tilt.x = (int) (asin(x) * 180 / M_PI) + 90;
	wiimote->tilt.y = (int) (asin(y) * 180 / M_PI) + 90;
	wiimote->tilt.z = (int) (asin(z) * 180 / M_PI) + 90;	
}
...
static int process_state(wiimote_t *wiimote, wiimote_state_t *ev)
{
	...
	case WIIMOTE_RID_ISTATUS:
		...
		/* Automatically initialize the nunchuk when plugged in. */
	...
	case WIIMOTE_MODE_EXT: /* 0x34 */
		...
		wiimote->keys.bits = ev->keys.bits;
		nunchuk_decode((uint8_t *) &ev->ext.nunchuk, 6);
		memcpy(&wiimote->ext.nunchuk, &ev->ext.nunchuk, sizeof (wiimote_nunchuk_t));
	...
	case WIIMOTE_MODE_ACC_EXT: /* 0x35 */
		...
		nunchuk_decode((uint8_t *) &ev->ext1.nunchuk, 6);
		memcpy(&wiimote->ext.nunchuk, &ev->ext1.nunchuk, sizeof (wiimote_nunchuk_t));
		wiimote->ext.nunchuk.keys.bits ^= 0xff;
		calc_tilt(wiimote);
	...
	case WIIMOTE_MODE_IR_EXT: /* 0x36 */
		...
		wiimote->keys.bits = ev->keys.bits;
		nunchuk_decode((uint8_t *) &ev->ext2.nunchuk, 6);
		memcpy(&wiimote->ext.nunchuk, &ev->ext2.nunchuk, sizeof (wiimote_nunchuk_t));
		wiimote->ext.nunchuk.keys.bits ^= 0xff;
	...
	case WIIMOTE_MODE_ACC_IR_EXT: /* 0x37 */
		...
		nunchuk_decode((uint8_t *) &ev->ext3.nunchuk, 6);
		memcpy(&wiimote->ext.nunchuk, &ev->ext3.nunchuk, sizeof (wiimote_nunchuk_t));
		wiimote->ext.nunchuk.keys.bits ^= 0xff;
